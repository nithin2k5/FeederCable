...
("Labels and Scanning", [
    ...
    ("h", "Every placeholder"),
    ("p", "The whole set, and which of the two templates each one is "
          "substituted in. A placeholder used on a template that does not "
          "support it is not an error and nothing warns about it — it simply "
          "prints as itself, @ signs and all, on every label that goes out."),
    ("tbl", ["Placeholder", "Part label", "Lot label"], [
        ["@partNumber@", "yes", "yes"],
        ["@modelName@", "yes", "yes"],
        ["@alcCode@", "yes", "yes"],
        ["@vendorCode@", "yes", "yes"],
        ...
        ["@lotNo@", "yes", "yes"],
        ["@lot3letters@", "yes", "yes"],
        ["@machineID_NoAlphabet@", "yes", "yes"],
        ["@ddMMyy@", "yes", "yes"],
        ["@HH:mm:ss@", "yes", "yes"],
        ["@traceabilityCode@", "yes", "—"],
        ["@irValues@", "yes", "—"],
        ["@acwValues@", "yes", "—"],
        ["@machineID@", "—", "yes"],
        ["@dd/MM/yy@", "—", "yes"],
    ...
    ("note", "@body@ is not in this list. It is the one placeholder in "
             "MARKER.prn, and the machine fills it with the whole text block "
             "of a START or END label — the lines and their positions are "
             "worked out in software, so nothing else in that file can be "
             "placeholdered."),
    ...
    ("tbl", ["Placeholder", "Replaced with"], [
        ...
        ["@lot3letters@", "The date as its three character code, year then month then day — one character each. Also accepted without the closing @"],
    ]),
    ...
    ("p", "@lot3letters@ prints today's date as three characters, most "
          "significant first: one for the year, one for the month, one for "
          "the day. 15 September 2026 is VIO — V for 2026, I for September, "
          "O for the 15th."),
    ("p", "The closing @ can be left off. A template written @lot3letters is "
          "substituted the same way, because an unclosed one would otherwise "
          "print as itself and the mistake is easy to make. No other "
          "placeholder is forgiving of this."),
    ("p", "The characters are not worked out by the machine, they are looked up "
          "in three files that sit with the label templates: DAY CODE.txt holds "
          "31 entries, MONTH CODE.txt holds 12, and YEAR CODE.txt holds one per "
          "year from 2021. Each is a single line of values separated by commas, "
          "in order, so the first entry in DAY CODE.txt is the code for the 1st "
          "and the first in YEAR CODE.txt is the code for 2021."),
    ("p", "As shipped the days run A to Z then 1 to 5, the months A to L, and "
          "the years Q to Z then A to J — 2021 to 2040. A customer who codes dates "
          "differently only needs these files edited — the tables are read again "
          "for every label, so a change applies to the next one printed without "
          "restarting. A full stop on the end of a line is ignored, so a table "
          "typed out as a sentence still reads correctly."),
    ("note", "A label reading ? in place of one of the three characters means "
             "the date fell outside its file — YEAR CODE.txt running out after "
             "2040 is the one to expect. Add the missing entries and reprint."),
